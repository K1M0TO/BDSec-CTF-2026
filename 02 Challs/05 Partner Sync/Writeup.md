...
페이지에 접속하면 임의의 URL에 요청을 보낼 수 있는 form이 존재한다. 가장 먼저 떠오른 것은 SSRF였다. 서버의 권한으로 내부 자원에 접근해 flag를 읽는 것이다.

그리고 주석을 확인하니 다음과 같이 실수로 노출된 파일에 대한 정보가 있었다.
```
...
`/static/js/ops-console.js`엔 레거시 빌드가 존재했고 서버 소스코드의 일부를 확인할 수 있었다.

그리고 접근해야 하는 대상인 두 개의 서비스에 대한 정보를 확인할 수 있다.
...
그 중에서 사용자 입력값을 검증하는 다음 함수엔 다음과 같은 나름의 필터링이 있었는데, 여기엔 취약점이 존재한다.
...
소스코드를 더 살펴보면 internal-api:9000에서 가능한 작업이 더 확인된다. 주석엔 system.run()을 실행할 수 있는 방법이 명시돼있다.

buildJob 파라미터인 `handler`는 system.run일 것이며, arg는 system.run()의 인자(= shell 명령)일 것이다. 이걸 custom encoding을 통해 internal-api:9000에 전송한다.
```
...
인공지능의 도움을 받아서 internal-api:9000/job 이라는 엔드포인트를 발견했지만 이는 POST method를 요구했다.

custom encoding을 수행하는 코드도 작성했지만, 문제는 이를 어떻게 body로 전달하냐는 것이었다.

CTF가 끝날 때까지 이 이상으로 진도를 나가는 것이 불가능했고, 이후 풀이자가 디스코드에 공유한 풀이 방법을 참고해 풀었다.
...
아무튼 burp suite를 이용해 /job 엔드포인트에 app.py를 읽는 요청을 전송한다.
```
...
그럼 결과로 app.py가 출력되는데, 중요한 내용만 추출하자면 다음과 같다.
...
- PREFIX 검증은 startWith()으로 할 경우, `@`로 우회할 수 있으니 host를 철저하게 구분하여 검증하자. 
